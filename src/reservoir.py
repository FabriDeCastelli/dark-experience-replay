...
class ReservoirBuffer:
    """
    Reservoir buffer for experience replay.
    """
    ...
    def add(self, x, z, y=None, task_id=None):
        ...
        if self.x_buffer is None:
            ...
            if y is not None:
                self.y_buffer = torch.empty((self.buffer_size,), dtype=torch.long)
    ...
    def sample(self, batch_size=1, transform=None):
        ...
        valid_y = self.y_buffer[self.is_filled]
        return apply_transform(valid_x[indices], transform), valid_z[indices], valid_y[indices], task_ids
